...
function AnimatedStat({ value, label, delay }: { value: string; label: string; delay: number }) {
  ...
  return (
    ...
    >
      <dt className="font-mono font-bold text-3xl lg:text-4xl text-text-primary mb-1 text-neon-glow-subtle">
        <span>{prefix}</span>
...
export default function Stats() {
  return (
    <section className="relative border-y border-neon/[0.06]" aria-label="Key metrics">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-10 lg:py-12">
        <dl className="grid grid-cols-2 sm:grid-cols-4 gap-8 lg:gap-12">
          {STATS.map((stat, i) => (
            <div
              key={stat.label}
              className={`text-center ${i < STATS.length - 1 ? "sm:border-r sm:border-neon/[0.06]" : ""}`}
            >
